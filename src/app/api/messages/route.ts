...
export async function POST(req:NextRequest) {
    try {
        ...
        return NextResponse.json({message:"Error in saving message",success:false},{status:500});
    }
}
...
export async function DELETE(request: NextRequest){
    ...
    try {
        ...
        if(deleteResult.count === 0){
            return NextResponse.json({message:"No message found to delete or unauthorized",success:false},{status:404});
        }

        await pub.publish("DELETE_MESSAGES",JSON.stringify({messageId,senderId}));
        ...
        return NextResponse.json({message:"Error in deleting message",success:false},{status:500});
    }
}