...
function Visualise() {
    ...
    const [parameter,setParameter]=useState<ImgParameter>({
        width:500,height:500,model:"flux",seed:50,nologo:true})
  ...
  const handleDownloadImage = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/download-image?url=${safeUrl}&filename=${encodeURIComponent(`${promt}.jpeg`)}`);
  ...
  return (
    <div className='flex flex-col space-y-5'>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:gap-4">
        ...
        <div className="w-64">
          <Select
            ...
            onValueChange={(val) => setParameter({...parameter,model:val as typeof parameter.model})}
          >
            <SelectTrigger>
              <SelectValue placeholder="Select model" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="flux" defaultChecked={true}>Flux</SelectItem>
      ...
      {showMessage && (
        <p className="text-green-600 text-sm font-medium"><Loader2 className='animate-spin'/>Generating...</p>
      )}
      <div className={`h-${parameter.height} w-${parameter.width} align-middle flex justify-center items-center`}>
